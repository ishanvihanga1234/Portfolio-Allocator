...
def tangency_portfolio(mu_pct, cov_pct, rf_pct, long_only=True):
    """
    ...
    long_only : if True (default), constrain weights to w >= 0 (no short-selling),
                solved via SLSQP maximizing the Sharpe ratio subject to sum(w)=1, w>=0.
                If False, use the closed-form unconstrained tangency formula
                w ~ Sigma^-1 (mu - rf), which can produce negative (short) weights.
    ...
    # Long-only max-Sharpe via SLSQP
    from scipy.optimize import minimize

    def neg_sharpe(w):
        ret = w @ mu_pct
        vol = np.sqrt(w @ cov_pct @ w)
        if vol <= 1e-12:
            return 1e6
        return -(ret - rf_pct) / vol
    ...
    if not result.success:
        raise ValueError(f"Long-only Markowitz optimization failed to converge: {result.message}")

    w = np.clip(result.x, 0, None)
    w = w / w.sum()  # re-normalize to guard against tiny numerical drift
    mu_p = w @ mu_pct
    sigma_p = np.sqrt(w @ cov_pct @ w)
    return w, mu_p, sigma_p
...
def run_allocation(portfolio_xlsx, dataset_xlsx, tenor_month, investment_amount,
                    preference_list=None, long_only=True):
    ...
    w, mu_p, sigma_p = tangency_portfolio(mu_pct, cov_pct, rf_pct, long_only=long_only)
