...
def parse_args():
    ...
    p.add_argument("--format", choices=["excel", "pdf", "both"], default="both",
                    help="Report format to generate (default: both)")
    p.add_argument("--long-only", action="store_true", default=True,
                    help="Disallow short-selling in Markowitz optimization (default: on)")
    p.add_argument("--allow-short", dest="long_only", action="store_false",
                    help="Allow short-selling (unconstrained tangency portfolio)")
    return p.parse_args()
...
def main():
    ...
    if args.format in ("excel", "both"):
        xlsx_path = os.path.join(args.outdir, f"Allocation_Report_{safe_name}_{tenor}_{ts}.xlsx")
        build_excel_report(res, amount, name, tenor, xlsx_path)
        print(f"\nExcel report saved: {xlsx_path}")

    if args.format in ("pdf", "both"):
        pdf_path = os.path.join(args.outdir, f"Allocation_Report_{safe_name}_{tenor}_{ts}.pdf")
        build_pdf_report(res, amount, name, tenor, pdf_path)
